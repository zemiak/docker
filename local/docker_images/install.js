...
Services.docker_images_run = function() {
    ...
    // base support
    CommandLineRunner.execute("mkdir -p /opt/docker_services");
    CommandLineRunner.execute("cp *.js /opt/docker_services/");
    
    if (Services.debianVersion === 7) {
        // Debian Wheezy (7.0) -- SysV init
        CommandLineRunner.execute("cp docker_services /etc/init.d/");
        ...
        CommandLineRunner.execute("cp docker_services.service /etc/systemd/system/");
